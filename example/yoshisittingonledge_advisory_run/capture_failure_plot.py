...
out_dir = Path(r'E:\automated-3d-model-splitting\example\yoshisittingonledge_advisory_run')
...
sys.argv = ['split_painted_3mf.py','--input',r'D:\3Dmodel\yoshisittingonledge.3mf','--output',str(out_dir/'yoshisittingonledge_split_parts.3mf'),'--region-review-json',str(out_dir.parent/'yoshisittingonledge_region_review_current'/'user_decisions.json'),'--recovery-dir',str(out_dir/'recovery'),'--connector-slope-validation','advisory','--surface-band-validation','advisory','--connector-surface-validation','advisory','--diagnostic-preview']
main()
