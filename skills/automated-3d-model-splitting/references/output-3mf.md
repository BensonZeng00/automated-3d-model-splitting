...
Default to a 3.0 mm preferred minimum and a measured safe maximum of `min(50 mm, active parent bounding-box diagonal, sampled parent thickness - 0.05 mm)`. Measure thickness and evaluate slope only on deterministic equal-arc samples of each interface boundary (at most 768 points); never use the raw boundary vertices for those evaluations. Connector depth rays combine the projected direction toward the interface-plane center with the local inward normal and search 30–75 degrees from that normal; no parent outer-surface hit within the finite search horizon means safe travel through that horizon. Generate actual backing geometry on the original shared boundary and map sampled depth limits to intervening geometry without claiming extra measurements. Permit different boundary-point depths when all bottom points lie on one coherent plane, and place that plane at the deepest position whose minimum and maximum travel remain inside the effective and safe bounds. Use local-offset at the measured safe maximum only when no coherent plane satisfies both bounds. Form the subsurface interface as an outer-large, inner-small taper and select jointly feasible depth and 30–75-degree slope from the same sampled boundary set. Record sample counts, measured parent thickness, reserve, preferred/effective minimum, safe maximum, required planar maximum, selected cap mode, minimum/maximum generated inward travel, and target/measured taper angles for every part.
