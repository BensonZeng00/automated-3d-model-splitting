...
class ConfirmedSeamTests(unittest.TestCase):
    def setUp(self):
        ...
        self.context = PlanarArcRetopologyContext(PlanarArcRetopologyConfig())

    def test_default_is_unchanged(self):
        self.assertFalse(hasattr(PlanarArcRetopologyConfig(), 'preserve_confirmed_seam'))
    ...
    def test_service_always_invokes_spline(self):
        ...
        with patch('split3mf.interface_retopology.build_planar_arc_boundary',
                   wraps=build_planar_arc_boundary) as spline:
            result, record = InterfaceRetopologyService.retopologize_loop(
                self.points, np.arange(4), self.context)
        self.assertEqual(spline.call_count, 1)
        self.assertEqual(record['guide_model'], 'least-squares-periodic-cubic-b-spline')
