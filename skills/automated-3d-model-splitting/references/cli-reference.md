...
Default cap and nested-cap modes are `adaptive`. Thickness rays and backing-slope evaluations use deterministic equal-arc interface-boundary samples, capped at 768 per ring. Connector depth searches combine the projected direction toward the interface-plane center with the local inward normal, checking rays 30–75 degrees from that normal; when no parent outer surface is hit, the finite search distance is safe travel. The safe depth ceiling is bounded by `min(50 mm, active parent bounding-box diagonal, sampled parent thickness - 0.05 mm)`. Geometry still uses the original shared boundary; mapped values between samples are construction inputs, not additional thickness or slope measurements. The preferred minimum is 3.0 mm unless sampled parent thickness leaves less safe depth. It first tries the deepest coherent plane whose sampled point depths remain within both effective and safe bounds. When no such plane fits, it uses local-offset at the measured safe maximum. The subsurface entry ring jointly selects depth and a 30–75-degree outer-large, inner-small taper from the sampled boundary. An explicit `--planar-extra-limit-mm` can narrow the global ceiling. There is no CLI fallback for center-fan caps; unsafe synthetic-hub closure is forbidden.
